...
`git clone https://github.com/ByteLegendQuest/java-fix-same-reference`

A project directory named `java-fix-same-reference` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-same-reference` directory which is created just now.
